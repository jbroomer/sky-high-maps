...
slope_file = io.open( '/home/ian/Desktop/osrm-backend-master/profiles/mass-slope.json', "r" )
...
function elevation_gain(way_info, forward)

    if forward then
        return way_info["climb"]
    else
        return way_info["descent"]
    end
...
function process_way(profile, way, result, relations)
  ...
  if (way_info~=nil) then
    local fr = 1 / elevation_gain(way_info, true)
    local br = 1 / elevation_gain(way_info, false)
    result.forward_rate = (fr^3) * 1000000000
    result.backward_rate = (br^3) * 100000000
  end
end
